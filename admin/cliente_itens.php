...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($acao === 'item_salvar') {
        ...
        $estSaldo = $isAdmin
            ? (float) str_replace(',', '.', (string) ($_POST['estoque_saldo'] ?? (string) $estCap))
            : ($id === null ? $estCap : null);
        ...
        $rec = repo_catalogo_recalcular_estoque_saldo($clienteId);
        if ($rec['ok']) {
...
$catalogoPainelAdminEditaSaldo = (($me['perfil'] ?? '') === 'admin');
