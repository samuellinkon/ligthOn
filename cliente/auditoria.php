<?php
/**
 * Auditoria — portal cliente (eventos da sua unidade).
 */
...
require_modulo_cliente('auditoria');
...
if ($exportCsv) {
    ...
    header('Content-Disposition: attachment; filename="auditoria_cliente_' . date('Y-m-d_His') . '.csv"');
...
function cli_auditoria_url(int $p, array $f): string
{
    $qs = array_filter([
        'de'   => $f['de'] ?? '',
        'ate'  => $f['ate'] ?? '',
...
$topTitle    = 'Auditoria';
$topSubtitle = 'Histórico de ações da sua unidade — filtro por datas e exportação CSV.';
$topSearch   = '';
$topAction   = null;
  ...
  <div class="card">
    <div class="panel-head">
      <div>
        ...
        <span class="panel-sub">Até <?= (int) AUDIT_LOG_EXPORT_MAX_ROWS ?> linhas no CSV. Mostram-se eventos da sua matriz e filiais, e ações suas quando o registo não tem empresa associada.</span>
      </div>
    ...
    <div class="panel-body">
      ...
      <nav class="pagination">
        <?php if ($page > 1): ?>
        <a class="pag-btn" href="<?= htmlspecialchars(cli_auditoria_url($page - 1, $filtros)) ?>">Anterior</a>
        <?php endif; ?>
        <span class="muted" style="padding:0 12px;">Página <?= (int) $page ?> / <?= (int) $totalPages ?></span>
        <?php if ($page < $totalPages): ?>
        <a class="pag-btn" href="<?= htmlspecialchars(cli_auditoria_url($page + 1, $filtros)) ?>">Seguinte</a>
        <?php endif; ?>
      </nav>
      <?php endif; ?>
      <?php endif; ?>
