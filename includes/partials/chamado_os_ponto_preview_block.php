 ...
 * O mapa fica oculto até o usuário clicar em «Atualizar mapa com endereço»
 * (ou já existirem coordenadas salvas / ponto vinculado).
 */
?>
<div class="form-group full os-ponto-preview-wrap" style="margin-top:8px;">
  ...
  <div id="os_ponto_preview" class="os-ponto-preview"<?= $ch_os_preview_inicial_visivel ? '' : ' hidden' ?>
    ...
    <p id="os_ponto_sem_coord" class="muted os-ponto-preview__hint" hidden>Três formas de localizar: CEP+endereço, só o endereço (cidade/UF ou texto completo), ou latitude/longitude — depois clique em «Atualizar mapa com endereço».</p>
    <div class="chamado-loc-view-bar" style="margin:0 0 8px;flex-wrap:wrap;gap:8px;" role="group" aria-label="Atualizar localização pelo endereço">
      <button type="button" class="btn btn-sm btn-primary" id="os_ponto_geocode_btn" title="Buscar no mapa com o endereço preenchido nos campos acima">
        Atualizar mapa com endereço
      </button>
    </div>
    <p id="os_ponto_geocode_hint" class="muted os-ponto-preview__hint" style="margin:0 0 8px;" hidden></p>
    <div id="os_ponto_streetview_block" class="chamado-ponto-streetview" hidden>
      ...
      <div class="chamado-ponto-streetview__frame-wrap">
        <iframe id="os_ponto_streetview_frame" class="chamado-ponto-streetview__frame" title="Localização do chamado no mapa" allowfullscreen loading="lazy" allow="accelerometer; gyroscope; magnetometer; fullscreen; geolocation; clipboard-write" referrerpolicy="no-referrer-when-downgrade" hidden<?= $ch_os_sv_iframe_src !== '' ? ' data-sv-embed-src="' . htmlspecialchars($ch_os_sv_iframe_src, ENT_QUOTES, 'UTF-8') . '"' : '' ?>></iframe>
        ...
        <?php if (!empty($ch_os_use_google_maps_embed)): ?>
        <iframe id="os_ponto_map_embed" class="chamado-map-embed-frame chamado-ponto-streetview__frame" title="Mapa do chamado" allowfullscreen loading="lazy" allow="accelerometer; gyroscope; magnetometer; fullscreen; geolocation; clipboard-write" referrerpolicy="no-referrer-when-downgrade" hidden<?= ($ch_os_map_embed_src ?? '') !== '' ? ' data-embed-src="' . htmlspecialchars((string) $ch_os_map_embed_src, ENT_QUOTES, 'UTF-8') . '"' : '' ?>></iframe>
