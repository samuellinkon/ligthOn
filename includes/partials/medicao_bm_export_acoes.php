...
<div class="actions-inline<?= $bmExportComLabels ? ' medicao-export-actions--labeled' : '' ?>">
  <?php if ($bmExportComLabels): ?>
    ...
    <button type="submit" form="<?= htmlspecialchars($bmFormId, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-secondary btn-sm js-crm-export-bm">BM</button>
    <a class="btn btn-secondary btn-sm js-medicao-periodo-link js-crm-export-link" href="<?= htmlspecialchars($hrefXlsxDet, ENT_QUOTES, 'UTF-8') ?>" data-link-kind="xlsx_detalhes">BM completo</a>
    <a class="btn btn-secondary btn-sm js-medicao-periodo-link js-crm-export-link" href="<?= htmlspecialchars($hrefPdfAnexos, ENT_QUOTES, 'UTF-8') ?>" data-link-kind="pdf_anexos" target="_blank" rel="noopener">Rel. fotográfico</a>
  <?php else: ?>
